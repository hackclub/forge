module DocsProps
  ...
  def docs_props(raw_slug)
    slug = raw_slug.to_s
    slug = "index" if slug.blank?
    return docs_not_found unless valid_docs_slug?(slug)
    ...
    return docs_not_found unless path
    return docs_not_found unless File.expand_path(path).start_with?(File.expand_path(base))

    file_meta = helpers.guide_metadata_for(path)
    meta = helpers.docs_meta_for_url("/docs#{slug == 'index' ? '' : "/#{slug}"}")
    page_title = meta&.dig(:title).presence || file_meta[:title].presence ||
  ...
  def docs_not_found
    {
      content_html: "<p>That page could not be found.</p>",
