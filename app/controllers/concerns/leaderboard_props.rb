module LeaderboardProps
  ...
  def leaderboard_props
    ...
    hours_ids = hours_totals.select { |_, v| v > 0 }.sort_by { |_, v| -v }.first(LEADERBOARD_LIMIT).map(&:first)
    streak_ids = streak_totals.sort_by { |_, v| -v }.first(LEADERBOARD_LIMIT).map(&:first)
    ...
    {
      guild_referrals: leaderboard_guild_referrals,
